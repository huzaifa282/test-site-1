...
// State to track mobile menu status
...
// Manage event listener for clicks outside the mobile menu
const manageClickOutsideEvent = (shouldAdd) => {
  if (process.client) {
    if (shouldAdd) {
...
// Watcher to toggle the event listener when `isMobileMenuOpen` changes
watch(isMobileMenuOpen, (newValue) => {
  manageClickOutsideEvent(newValue);
});

// Clean up event listener on unmount
onBeforeUnmount(() => {
  manageClickOutsideEvent(false);
});
</script>
...
main {
  margin-top: 80px; /* Adjust for the height of the fixed navigation */
  padding: 20px;
}
