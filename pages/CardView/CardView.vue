...
import posts from '~/plugins/source';
  ...
  export default {
    ...
    computed: {
        currentPost() {
            const post = posts.find(post => slugify(post.name) === this.slug);
  if (!post) {
    console.warn(`Post not found for slug: ${this.slug}`);
    return { name: 'Post Not Found', contents: 'This post could not be found.' };
  }
  return post;
        },
        popularPosts(){
    ...
    console.error('Image not found:', currentPost.value.image_src);
    return '/public/images/default.jpg'; // Fallback image
  }
        },
        multipleDownload(){
